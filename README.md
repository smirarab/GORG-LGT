# GORG-LGT
Analysis of LGT across the GORG Tropics dataset. Five sets of analyses are provided here in the directories below. 

* Each directory has **its own README.md** file showing:
	* Requirements
	* Content of files
* We have packaged the data and code with each other. You can run scripts in each directory using data in that directory
* None of the software tools require installation, just the requirements. 

Here are the directories  

* For simulations of LGT-free genomes (Figure S6 and S1), see [GNDSim](GNDSim/)
* For tabular comparison of the LGT-free genomes see [ORFvANI](ORFvANI)
* For Figures 1b, 1c, and supplementary Figures S3 and S5, see [GNDModel](GNDModel/)
* For Figure 3a and S4, S7 see [genetreeAAD](genetreeAAD/)
* For Figure 4c, see [16SAAD](16SAAD/)

